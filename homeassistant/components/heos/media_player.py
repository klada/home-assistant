...
class HeosMediaPlayer(MediaPlayerEntity):
    ...
    @log_command_error("join")
    async def async_join(self, group_members):
        """Join `group_members` as a player group with the current player."""
        player_ids = []
        for player_entity_id in group_members:
            for entity in self.hass.data[DOMAIN].entities:
                if isinstance(entity, HeosMediaPlayer) and entity.entity_id == player_entity_id:
